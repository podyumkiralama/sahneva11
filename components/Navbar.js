...
// 🧮 Yardımcılar
function calcPrice(pkg) {
  const area = parseFloat(pkg.specs.area);
  let base;
  switch (pkg.specs.pixel) {
    case "P2.5": base = area * PRICING.p2_5; break;
    case "P3.9": base = area * PRICING.p3_9; break;
    case "P4": base = area * PRICING.p4; break;
    default: base = area * PRICING.p6;
  }
  return {
    base: Math.round(base),
    setup: PRICING.setup,
    operator: PRICING.operator,
    total: Math.round(base + PRICING.setup + PRICING.operator),
  };
}
const enrichedPackages = PREMIUM_CONTENT.packages.map((pkg) => ({ ...pkg, pricing: calcPrice(pkg) }));
const formatTRY = (n) => new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY", maximumFractionDigits: 0 }).format(n);

// 🎨 Buton stilleri
const BTN = {
  primary: "inline-flex items-center justify-center font-bold px-8 py-4 rounded-2xl bg-gradient-to-r from-blue-700 to-purple-700 text-white transition-all duration-300 hover:shadow-2xl focus:outline-none focus:ring-4 focus:ring-blue-300 focus:ring-offset-2 focus:ring-offset-blue-900",
  outline: "inline-flex items-center justify-center font-bold px-8 py-4 rounded-2xl border-2 border-white text-white bg-white/10 backdrop-blur-sm hover:bg-white/20 transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-white focus:ring-offset-2 focus:ring-offset-blue-900",
  success: "inline-flex items-center justify-center font-bold px-8 py-4 rounded-2xl bg-gradient-to-r from-green-600 to-emerald-700 text-white transition-all duration-300 hover:shadow-2xl focus:outline-none focus:ring-4 focus:ring-green-300 focus:ring-offset-2 focus:ring-offset-green-900",
};
    ...
    <div className="min-h-screen bg-white">
      ...
      >
        ...
        <div className="relative z-10 container mx-auto px-4 text-center text-white py-16">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16" aria-label="Birincil eylemler">
            <Link href="#paketler" className={BTN.primary}>
              <span aria-hidden="true">🖥️</span>
              <span className="ml-2">Paketleri İncele</span>
            </Link>
            <a href="tel:+905453048671" className={BTN.success}>
              <span aria-hidden="true">📞</span>
              <span className="ml-2">Hemen Teklif Al</span>
            </a>
            <Link href="#teknoloji" className={BTN.outline}>
              <span aria-hidden="true">⚡</span>
              <span className="ml-2">Teknoloji</span>
            </Link>
          </div>

          <ul className="flex flex-wrap justify-center items-center gap-8 text-white/90 text-sm drop-shadow">
      ...
      <main id="main-content" tabIndex={-1}>
          ...
          <div className="container mx-auto px-4">
            ...
            <ul className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {PREMIUM_CONTENT.features.map((feature) => (
                <li
                  key={feature.title}
                  className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 text-center hover:shadow-xl transition-all duration-300"
                >
                  <div className="text-3xl mb-4 transition-transform duration-300" aria-hidden="true">
                    {feature.icon}
                  </div>
        ...
        <section id="paketler" className="py-20 bg-gradient-to-b from-gray-50 to-blue-100/20" aria-labelledby="packages-heading">
          <div className="container mx-auto px-4">
            ...
            <ul className="grid lg:grid-cols-3 gap-10 max-w-6xl mx-auto">
              {enrichedPackages.map((pkg) => (
                <li key={pkg.id}>
                  <article
                    className={`relative bg-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500 ${
                      pkg.badge === "Çok Satan"
                        ? "ring-4 ring-blue-500/20 scale-105 hover:scale-110"
                        : "hover:-translate-y-2"
                    }`}
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <a
                        href={`https://wa.me/905453048671?text=${encodeURIComponent(`Merhaba, ${pkg.name} hakkında detaylı bilgi ve teklif almak istiyorum.`)}`}
            ...
            <div className="text-center mt-16 max-w-2xl mx-auto">
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all duration-300">
              ...
              <div className="bg-gradient-to-br from-purple-50 to-white rounded-2xl p-6 border border-purple-200 hover:shadow-lg hover:border-purple-300 transition-all duration-300">
                ...
                <ul className="grid gap-4">
                  {[
                    ...
                    <li key={index} className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-purple-300 hover:shadow-md transition-all duration-300">
