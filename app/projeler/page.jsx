    ...
    <main id="main" className="min-h-screen bg-gradient-to-br from-slate-900 via-[#0b0f1a] to-purple-900/20 text-white">
      ...
      <section className="relative mx-auto max-w-7xl px-4 pt-20 md:pt-24 pb-20">
        ...
        <div className="relative">
          <ul role="list" aria-label="Proje listesi" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {projects.map((p, index) => (
              <li key={p.slug} className="group relative" style={{ animationDelay: `${index * 100}ms` }}>
                ...
                <Link
                  ...
                  className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-500 hover:scale-[1.02] hover:border-white/20 hover:shadow-2xl hover:shadow-blue-500/10"
                ...
                >
                  <div className="relative aspect-[16/10] overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                      <span className="inline-flex items-center gap-2 rounded-full bg-white/20 px-6 py-3 font-semibold text-white">
                        Projeyi İncele
                  ...
                  <div className="flex flex-1 flex-col gap-4 p-6">
                    <div className="flex-1">
                      <div className="mb-3 flex items-start justify-between gap-2">
                        <h2 className="text-xl font-bold leading-tight text-white line-clamp-2">{p.title}</h2>
                        <svg className="mt-1 h-5 w-5 flex-shrink-0 text-white/40 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                        </svg>
                      </div>
                      {p.date && (
                        <time className="mb-3 inline-flex items-center gap-1 text-sm text-white/60" dateTime={p.date}>
                      ...
                      {p.excerpt && (
                        <p className="text-sm leading-relaxed text-white/80 md:text-base line-clamp-3">{p.excerpt}</p>
                      )}
                    </div>
                    {p.tags?.length > 0 && (
                      <ul className="flex flex-wrap gap-2" aria-label="Proje etiketleri">
                        {p.tags.slice(0, 3).map((t) => (
                          <li key={t} className="inline-flex items-center rounded-full border border-blue-500/30 bg-gradient-to-r from-blue-500/20 to-purple-500/20 px-3 py-1 text-xs font-medium text-blue-200">
                            <span>#{t}</span>
                          </li>
                        ))}
                        {p.tags.length > 3 && (
                          <li className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-xs font-medium text-white/60">
                            <span>+{p.tags.length - 3}</span>
                          </li>
                        )}
                      </ul>
                    )}
                    <span className="inline-flex items-center justify-center rounded-full border border-white/20 px-4 py-2 text-sm font-semibold text-white transition-colors duration-300 group-hover:border-white group-hover:bg-white/10">
                      Devamını Oku
                    </span>
                  </div>
                </Link>
