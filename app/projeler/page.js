...
// (İsteğe bağlı) Node API kullanımı garanti olsun:
      ...
      <section className="mx-auto max-w-7xl px-4 py-16">
        <header className="mb-10">
          ...
          <p className="mt-3 text-gray-300 max-w-3xl">
            Liste, <code className="px-1 rounded bg-white/10">/app/projeler</code>{" "}
            altındaki klasörleri otomatik algılar ve kapak görselini{" "}
            <code className="px-1 rounded bg-white/10">/public/projeler/&lt;slug&gt;/cover.*</code>{" "}
            dosyalarından alır.
          </p>
        </header>
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" role="list" aria-label="Proje listesi">
          {projects.map((p) => (
            <li key={p.slug} className="group rounded-2xl bg-white/5 ring-1 ring-white/10 hover:ring-white/20 transition focus-within:ring-white/40">
              <Link href={`/projeler/${p.slug}`} className="flex flex-col h-full focus:outline-none" title={`${p.title} projesini görüntüle`}>
