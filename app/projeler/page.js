      ...
      <section className="mx-auto max-w-7xl px-4 py-16">
        <header className="mb-10">
          ...
          <p className="mt-3 text-gray-300 max-w-3xl">
            Proje arşivimiz. Aşağıdaki liste, <code className="px-1 rounded bg-white/10">/app/projeler</code> klasörü altına eklediğiniz
            her proje klasörünü otomatik algılar ve kapak görselini <code className="px-1 rounded bg-white/10">/public/projeler/&lt;slug&gt;/cover.*</code>
            dosyalarından alır.
          </p>
        </header>
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" role="list" aria-label="Proje listesi">
          {projects.map((p) => (
            <li key={p.slug} className="group rounded-2xl bg-white/5 ring-1 ring-white/10 hover:ring-white/20 transition focus-within:ring-white/40">
              <Link
              ...
              >
                ...
                <div className="p-4 flex-1 flex flex-col gap-2">
                  ...
                  {p.date && <time className="text-sm text-white/60" dateTime={p.date}>{new Date(p.date).toLocaleDateString("tr-TR")}</time>}
